...
export default function CommentsSection({ orderId, onError }: CommentsSectionProps) {
  const { user } = useAuth()
  ...
  const load = useCallback(() => {
    api.get(`/comments/service-order/${orderId}`)
  ...
  const submitComment = async () => {
    ...
    setSubmitting(true)
    try {
      await api.post(`/comments/service-order/${orderId}`, { content: newComment })
      setNewComment('')
      load()
    } catch (err: any) {
